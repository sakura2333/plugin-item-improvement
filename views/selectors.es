...
const baseImprovementDataSelector = createSelector(
    ...
    (db, $const, chains, uniqMap) => {

        // ✅ 仅此 selector 使用本地 nedb
        const ARSENAL_NEDB_PATH = path.join(__dirname, '../assets/arsenal.nedb')
        const ITEMS_NEDB_PATH = path.join(__dirname, '../assets/items.nedb')
